...
int usage(const char *progname, int err)
{
	cerr << "Usage: " << progname << " [options] list_gmm list_of_test_files list_of_candidates\n\n";
				 ...
				 vector<string> &gmm_filenames, string &world_name)
{
	...
	while ((option = getopt(ArgC, (char **)ArgV, "d:e:D:E:w:")) != -1)
	{
		...
		{
		case 'd':
			input_dirs.push_back(optarg);
			break;
		case 'e':
			input_exts.push_back(optarg);
			break;
		case 'D':
			gmm_dirs.push_back(optarg);
			break;
		case 'E':
			gmm_exts.push_back(optarg);
			break;
		case 'w':
			world_name = optarg;
			break;
	...
	if (gmm_dirs.empty())
		gmm_dirs.push_back("./");
	if (input_exts.empty())
		input_exts.push_back(DEF_FEAT_EXT);
	if (gmm_exts.empty())
		gmm_exts.push_back(DEF_GMM_EXT);
	...
	{
		if (!input_dirs[i].empty() and *(input_dirs[i].rbegin()) != '/')
			input_dirs[i] += '/';
		if (!gmm_dirs[i].empty() and *(gmm_dirs[i].rbegin()) != '/')
			gmm_dirs[i] += '/';
		if (!input_exts[i].empty() and input_exts[i][0] != '.')
			input_exts[i] = '.' + input_exts[i];
		if (!gmm_exts[i].empty() and gmm_exts[i][0] != '.')
