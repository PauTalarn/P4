				 ...
				 int &init_method, unsigned int &verbose)
{
	...
	while ((option = getopt(ArgC, (char **)ArgV, "d:e:m:g:n:N:t:T:i:v:F")) != -1)
	{
		...
		case 'g':
			...
			break;
		case 'n':
			init_iterations = atoi(optarg);
			break;
		case 'N':
			em_iterations = atoi(optarg);
			break;
		case 't':
			init_threshold = atof(optarg);
			break;
		case 'T':
			em_threshold = atof(optarg);
			break;
		case 'i':
			init_method = atoi(optarg);
			break;
		case 'v':
			verbose = atoi(optarg);
			break;
	...
	if (!input_dir.empty() and *input_dir.rbegin() != '/')
		input_dir += '/';
	if (!input_ext.empty() and input_ext[0] != '.')
		input_ext = '.' + input_ext;
