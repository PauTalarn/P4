/* Copyright (C) Universitat Politcnica de Catalunya, Barcelona, Spain.
 *
	...
	*/
	...
	int GMM::em(const fmatrix &data, unsigned int max_it, float inc_threshold, int verbose)
	{
		...
		for (iteration = 0; iteration < max_it; ++iteration)
		{
			...
			new_prob = em_expectation(data, weights); // devuelve la log probabilidad del material de entrenamiento
